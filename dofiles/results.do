...
* Table 4 and 5
	...
	using "${directory}/outputs/Table_5.xlsx" ///
		[pweight=weight_city] ///
	, p ci treatment(sp_male) controls(city_? case_?) title("Title") cl(facilitycode)
...
* Table 6
  	...
  	forvalues i = 1/4 {
		...
		chartable ///
			correct? ///
			using "${directory}/outputs/Table_6_`i'.xlsx" ///
			[pweight=weight_city] ///
			, $graph_opts command(logit) or rhs(any_male city_2 city_3 case_1 case_2 case_3 case_4 cp_5) xsize(10) case0(F-SP`i') case1(M-SP`i') ///
...
* Table 7
	...
	clonevar c1 = correct if cp_5  == 1
	clonevar c2 = correct if cp_5  == 0
	clonevar c3 = correct if cp_18 == 1
	clonevar c4 = correct if cp_18 == 0
	...
	rctreg ///
		...
		[pweight=weight_city] ///
		, p ci treatment(sp_male) controls(city_? case_? cp_5) title("Key Outcomes")
