...
syntax varlist using [if/] [in] [pweight/], ///
	controls(varlist) treatment(varlist) [sd] ///
	[title(string asis)] [round(real 0.01)] ///
	[lag] [iv(string asis)] [p] [se] [ci] [logit] *
	
local reg "reg"
if "`logit'" != "" local reg logit
if "`logit'" != "" local options "`options' or"
...
qui foreach var of varlist `varlist' {
	...
	* noi di "`reg' `var' `treatment' `morevars' `ifif' `if' `in' `weight', `options'"
	`reg' `var' `treatment' `morevars' `ifif' `if' `in' `weight', `options'
	...
	if "`iv'" != "" {		
		...
		ivregress 2sls `var' (`iv' = `treatment') `morevars' `ifif' `if' `in' `weight', `options'
	...
	`reg' `var' `treatment' `morevars' `controls' `ifif' `if' `in' `weight', `options'
	...
	if "`iv'" != "" {		
		...
		ivregress 2sls `var' (`iv' = `treatment') `morevars' `controls' `ifif' `if' `in' `weight', `options'
